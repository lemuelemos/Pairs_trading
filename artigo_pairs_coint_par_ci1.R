...
cat("\r","Agruping the data by pair",append = T)
...
for(ii in c(1,3)){
  if(ii == 1){
    cat("\r","Trading Period top 20 return","\r",append = T)
...
################ Cáculo dos Retornos Totais, Desvios Padrões e Sharpe.
cat("\n",paste0("Calculating return and sharpe. Portfolio ",p),append = T)
...
portfolios[[p]] <- invest_t_ci1
 ...
 names(save_data) <- paste0("Formatrion Window",formation_windown[pp]," dias uteis")
